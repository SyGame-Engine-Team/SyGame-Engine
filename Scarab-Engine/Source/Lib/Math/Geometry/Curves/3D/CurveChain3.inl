/////////////////////////////////////////////////////////////////////////////////
// File : Lib/Math/Geometry/Curves/3D/CurveChain3.inl
/////////////////////////////////////////////////////////////////////////////////
// Version : 1.0a
...
TCurveChain3<Real>::TCurveChain3( const Real * arrParameters, UInt iPatchCount ):
    TCurve3<Real>( arrParameters[0], arrParameters[iPatchCount] )
{
...
template<typename Real>
TCurveChain3<Real>::~TCurveChain3()
{
...
template<typename Real>
Real TCurveChain3<Real>::Length( Real fT0, Real fT1 ) const
{
...
template<typename Real>
Real TCurveChain3<Real>::Parameter( Real fLength, UInt iMaxIterations, Real fTolerance ) const
{
...
template<typename Real>
inline UInt TCurveChain3<Real>::GetPatchCount() const {
    return m_iPatchCount;
}
template<typename Real>
inline Void TCurveChain3<Real>::GetPatch( UInt iPatch, Real & outMinT, Real & outMaxT ) const {
    Assert( iPatch < m_iPatchCount );
    outMinT = m_arrParameters[iPatch];
    outMaxT = m_arrParameters[iPatch + 1];
}
template<typename Real>
Void TCurveChain3<Real>::SetPatch( UInt iPatch, Real fMinT, Real fMaxT )
{
    Assert( iPatch < m_iPatchCount );
    Assert( fMinT < fMaxT );
    if ( iPatch == 0 ) {
...
template<typename Real>
inline Real TCurveChain3<Real>::_SpeedFunction( Real fT, Void * pUserData ) {
    _PatchUserData * pPUD = (_PatchUserData*)pUserData;
    return pPUD->pThis->_GetPatchSpeed( pPUD->iPatch, fT );
}

template<typename Real>
Void TCurveChain3<Real>::_GetPatchInfo( Real fT, UInt & outPatch, Real & outDT ) const
{
